...
fn main() {
    ...
    let p: Vec<_> = {
        ...
        e.generate(10000);
        (1..2).chain(e.primes().iter().cloned()).collect()
    };
    input!(sc = sc, t: usize);
    for _ in 0..t {
        input!(sc = sc, n: usize, s: Chars);
        let cumsum: Vec<_> = (0..1)
            .chain(
                s.iter()
                    .map(|&c| if c == 'D' { 1 } else { 0 })
                    .scan(0, |c, x| {
                        *c += x;
                        Some(*c)
                    }),
        ...
        let mut ans = vec![1; n];
        for &prime in p.iter().rev() {
            if n < prime {
                continue;
            }
            let buff = cumsum[prime];

            for i in 2..n + 1 {
                if n < prime * i {
                    break;
                }
                let d = cumsum[prime * i] - cumsum[prime * (i - 1)];
                if buff * (prime - d) == d * (prime - buff) {
                    ans[prime * i - 1] = i;
                } else {
                    break;
                }
