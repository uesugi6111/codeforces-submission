...
fn main() {
    ...
    input!(sc = sc, t: usize);
    for _ in 0..t {
        input!(sc = sc, n: usize, w: i64, a: [i64; n]);
        let mut a = a;
        a.sort_unstable_by(|a, b| b.cmp(a));
        let mut ans = 0;
        let mut v: BTreeMap<i64, i64> = std::collections::BTreeMap::new();

        for i in 0..n {
            let mut is_ok = false;
            for &k in v.keys() {
                if k >= a[i] && *v.get(&k).unwrap() > 0 {
                    v.entry(k).and_modify(|x| *x -= 1);

                    v.entry(k - a[i]).and_modify(|x| *x += 1).or_insert(1);
            ...
            if !is_ok {
                v.entry(w - a[i]).and_modify(|x| *x += 1).or_insert(1);
                ans += 1;
            }
        }
