...
fn main() {
    ...
    for _ in 0..t {
        input!(sc = sc, n: usize, aa: [i64; n]);
        let mut a: std::collections::VecDeque<i64> = std::collections::VecDeque::new();
        let mut index = n - 1;
        let mut memo = vec![false; n];

        let mut max = n as i64;

        let mut buff = std::collections::VecDeque::new();
        while index >= 0 {
            buff.push_front(aa[index]);
            memo[(aa[index] - 1) as usize] = true;

            if aa[index] == max {
                a.append(&mut buff);
                buff = std::collections::VecDeque::new();
                while max > 0 && memo[max as usize - 1] {
                    max -= 1;
                }
            }
            if index == 0 {
