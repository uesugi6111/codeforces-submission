...
fn main() {
    ...
    for _ in 0..t {
        input!(sc = sc, n: usize, k: usize, a: [i64; n]);
        let mut set: std::collections::HashSet<_> = a.iter().map(|x| *x).collect();
        let mut is_loop = false;
        let mut max = *a.iter().max().unwrap();
        let mut mex = 0;
        for _ in 0..k.min(2 * n) {
            loop {
                if set.contains(&mex) {
                    mex += 1;
                } else {
        ...
        println!("{}", if is_loop { set.len() } else { n + k });
    }
}
